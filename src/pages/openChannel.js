...
export default class OpenChannel extends Component {
    ...
    channel.enter(function (response, error) {
      if (error) {
        if (error.code === 900100) {
          Alert.alert('Oops...You got banned out from this channel.')
        } else {
          Alert.alert('Enter openChannel Fail.')
        }
      }
      _SELF.props.navigation.navigate('chat', {channel: channel, refresh: _SELF._refreshChannelList})
    })
  }
  ...
  _onCreateOpenChannel () {
    this.props.navigation.navigate('CreateChannel', { refresh: this._refreshChannelList })
  }
