...
export default class GroupChannel extends Component {
  ...
  _onHideChannel (channel) {
    this.setState({
      channelList: this.state.channelList.filter((ch) => {
        return channel.url !== ch.url
      })
    }, () => {
      this.setState({
      ...
      Alert.alert(
        ...
        [
          ...
          },
          {
            text: 'Create',
            onPress: () => {
              _SELF.props.navigation.navigate('InviteUser', { _onHideChannel: _SELF._onHideChannel, refresh: _SELF._refreshChannelList })
            }
          },
          { text: 'Cancel' }
        ]
